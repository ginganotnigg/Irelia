...
func (qq *QuestionQuery) QueryInterview() *InterviewQuery {
	query := (&InterviewClient{config: qq.config}).Query()
...
func (qq *QuestionQuery) WithInterview(opts ...func(*InterviewQuery)) *QuestionQuery {
	query := (&InterviewClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withInterview = query
	return qq
}
...
func (qq *QuestionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Question, error) {
	...
	if query := qq.withInterview; query != nil {
		if err := qq.loadInterview(ctx, query, nodes, nil,
			func(n *Question, e *Interview) { n.Edges.Interview = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (qq *QuestionQuery) loadInterview(ctx context.Context, query *InterviewQuery, nodes []*Question, init func(*Question), assign func(*Question, *Interview)) error {
...
func (qq *QuestionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := qq.ctx.Fields; len(fields) > 0 {
		...
		if qq.withInterview != nil {
			_spec.Node.AddColumnOnce(question.FieldInterviewID)
		}
	}
