...
func (iq *InterviewQuery) QueryQuestions() *QuestionQuery {
	query := (&QuestionClient{config: iq.config}).Query()
...
func (iq *InterviewQuery) QueryFavorites() *InterviewFavoriteQuery {
	query := (&InterviewFavoriteClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iq *InterviewQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (iq *InterviewQuery) FirstIDX(ctx context.Context) string {
...
func (iq *InterviewQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (iq *InterviewQuery) OnlyIDX(ctx context.Context) string {
...
func (iq *InterviewQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (iq *InterviewQuery) IDsX(ctx context.Context) []string {
	ids, err := iq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (iq *InterviewQuery) WithQuestions(opts ...func(*QuestionQuery)) *InterviewQuery {
	query := (&QuestionClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withQuestions = query
	return iq
}
...
func (iq *InterviewQuery) WithFavorites(opts ...func(*InterviewFavoriteQuery)) *InterviewQuery {
	query := (&InterviewFavoriteClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withFavorites = query
	return iq
}
...
//		GroupBy(interview.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Interview.Query().
//		Select(interview.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (iq *InterviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Interview, error) {
	var (
		...
		_spec       = iq.querySpec()
		loadedTypes = [2]bool{
			iq.withQuestions != nil,
			iq.withFavorites != nil,
		}
	)
	...
	if query := iq.withQuestions; query != nil {
		if err := iq.loadQuestions(ctx, query, nodes,
			func(n *Interview) { n.Edges.Questions = []*Question{} },
			func(n *Interview, e *Question) { n.Edges.Questions = append(n.Edges.Questions, e) }); err != nil {
			return nil, err
		}
	}
	if query := iq.withFavorites; query != nil {
		if err := iq.loadFavorites(ctx, query, nodes,
			func(n *Interview) { n.Edges.Favorites = []*InterviewFavorite{} },
			func(n *Interview, e *InterviewFavorite) { n.Edges.Favorites = append(n.Edges.Favorites, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *InterviewQuery) loadQuestions(ctx context.Context, query *QuestionQuery, nodes []*Interview, init func(*Interview), assign func(*Interview, *Question)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Interview)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(question.FieldInterviewID)
	}
	query.Where(predicate.Question(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(interview.QuestionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "interview_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (iq *InterviewQuery) loadFavorites(ctx context.Context, query *InterviewFavoriteQuery, nodes []*Interview, init func(*Interview), assign func(*Interview, *InterviewFavorite)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Interview)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(interviewfavorite.FieldInterviewID)
	}
	query.Where(predicate.InterviewFavorite(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(interview.FavoritesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (iq *InterviewQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(interview.Table, interview.Columns, sqlgraph.NewFieldSpec(interview.FieldID, field.TypeString))
	_spec.From = iq.sql
